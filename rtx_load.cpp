...
std::map<std::string, Colour> load_mtl(std::string filename){
    ...
    if (file.is_open()){ //convert file to vector of strings
        std::string line;
    ...
    for(int i = 0; i < lines.size(); i++){ //for each line in file
        if (lines[i].length() < 1){
...
std::vector<ModelTriangle> load_files(std::vector<std::string> filenames){
    ...
    for(std::string filename : filenames){
        ...
        for(int i = 0; i < lines.size(); i++){ //for each line in file
            ...
            if(mode == "f"){
                if(splits.size() != 4) throw "Incorrect face information in object file!";
        ...
        for(std::string f : fs){
            ...
            for (int i = 1; i < 4; i++){ //process 3 vertex groups
            ...
            if(face_info.size() == 5){ //if face has a colour appended, try to locate it
